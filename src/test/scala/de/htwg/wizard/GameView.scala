...
class GameViewTest extends AnyWordSpec with Matchers {
  ...
  "GameView" should {
    ...
    "print askPlayerAmount prompt" in {
      val output = captureOutput { view.askPlayerAmount() }
    ...
    "print stitch winner" in {
      val output = captureOutput {
        view.showStitchWinner(player, Card(CardColor.Yellow, 3))
      }
