...
class GuiView(control: GameControl) extends Observer {
  ...
  private val sbHeight = 74
  private val sbWidth = 190

  private val cardWidthTrick = 150
  private val cardWidthHand = 130
  ...
  def showStart(primaryStage: Stage): Unit =
    stage = primaryStage
    stage.getIcons.add(new Image(icon)) // changes the programm icon
              ...
              children =
                activePlayer.hand.zipWithIndex.map { (card, idx) =>
                  new Button {
                    ...
                    disable = !control.isAllowedMove(card, activePlayer, currentTrick)
                    onAction = _ =>
  ...
  private def endScene(winner: Player, state: GameState): Scene =
    styledScene(
      new VBox {
        ...
        spacing = 20
        children = Seq(
          new Label(s"Winner: Player ${winner.id}") { styleClass += "winner-title" },
          ...
          new VBox {
            ...
            children =
              state.players
                ...
                .map(p => new Label(s"Player ${p.id}: ${p.totalPoints} points"){
                  styleClass += "text-label"
                })
          },
          new Separator(),
  ...
  private def trumpLabel(trump: Option[CardColor]): Label =
    ...
    val l = new Label(trump.map(c => s"Trumpf: $c").getOrElse("Trumpf: Kein"))
    l.styleClass += "trump-label"
    l
  ...
  private def cardNode(card: Card, cardWidth: Double): StackPane =
    ...
    new StackPane {
      prefWidth = cardWidth
      prefHeight = cardWidth * 1.45 // Kartenverhältnis
